...
fn fs( @builtin(position) pos : vec4f ) -> @location(0) vec4f {
    ...
    // mix effects with video based on multiplier
    let pain_multiplier = saturate((screen_brightness - MIX_LOW) / (MIX_HIGH - MIX_LOW));

    position.x /= resolution.x / resolution.y;
    position.x += sin((position.x + position.y) * 25. + frame / 7.5) * pain_multiplier * 0.015;

    // sample video
    var video = textureSampleBaseClampToEdge( videoBuffer, videoSampler, position);

    // bump contrast
    video = (video - 0.5) * (1. + pain_multiplier * 5.) + 0.5;

    return mix(video, vec4(pain_out, 1.), pain_multiplier);
